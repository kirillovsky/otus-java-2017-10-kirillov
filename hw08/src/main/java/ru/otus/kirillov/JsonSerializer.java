 ...
 * + По тому же принципу, поля родительских классов.
 * Циклические ссылки резолвить не будем, так как ответственность за такое использование лежит на пользователе.
 * Даже Gson резолвит только прямые циклы (a <=> a), циклы через один и более (a -> b ->.... -> a) - уже нет
 * 5. Наследники классов {@link java.util.Collection} и {@link java.util.Map}
 * Не будут сериализованы (т.е будет возвращен null) для:
 * 1. Объектов локальных классов
 * 2. Объектов нестатических классов-членов
