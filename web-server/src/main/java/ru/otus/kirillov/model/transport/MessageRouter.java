...
public class MessageRouter implements Observer<Response> {
    ...
    private final BlockingQueue<Response> incomingResponses;
    private final Map<Header, Observer<Response>> messageCorrelations;
    private final Map<Class<? extends Request>, DuplexChannel<Request, Response>> channelsMap;

    public MessageRouter(Map<Class<? extends Request>, DuplexChannel<Request, Response>> channelsMap,
                         ExecutorService service, int workersCount) {
        messageCorrelations = new ConcurrentHashMap<>();
        incomingResponses = new LinkedBlockingQueue<>();
        this.channelsMap = new HashMap<>(CommonUtils.retunIfNotNull(channelsMap));
        subscribeToAllChannels();
        initWorkers(service, workersCount, () -> {
            Response rs = incomingResponses.take();
    ...
    private void subscribeToAllChannels() {
        channelsMap.forEach((clazz, channel) -> channel.subscribe(this));
    }

    public void send(Observer<Response> sender, Request rq) {
        //1. Ищем канал если не нашли, кидаем экспешн
        DuplexChannel<Request, Response> channel = channelsMap.computeIfAbsent(rq.getClass(), (c) -> {
